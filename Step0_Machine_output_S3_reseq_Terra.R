...
# cd ~/GeoMx
# mkdir data
# gsutil cp -r gs://fc-secure-436590b2-8ffb-479d-bb93-e54aa6007b63/data/Oxford/OPDC/Transcriptomics/DA48_SpatialT_Geomx_v2/Data/40-1098621046/DCC/ data/        
#                 fc-secure-436590b2-8ffb-479d-bb93-e54aa6007b63

# stage the edited codes
cd ../practice/
git add Step0_Machine_output_S3_reseq_Terra.R 
git status
# to stage all the edited scripts
git add Step0_Machine_output_S3_reseq_Terra.R # to stage a specific script

git config --global user.email "isar.nassiri@well.ox.ac.uk"
git config --global user.name "isarnassiri"

# commit. 
git commit -m "commit"     # commit the staged scripts

# push to github
git push origin main                 # push to GitHub your "new scripts"

system(command = "gsutil cp  output/*  `gs://fc-317f434c-fe5c-4f4e-85d3-cf35e7f9dabe/$OWNER_EMAIL`")

git remote show origin

#============================================ 

datadir <- file.path("/home/rstudio/GeoMx/data/DCC/")
setwd(datadir)

DCCFiles <- dir(file.path(datadir, "dccs"), pattern = ".dcc$",
                full.names = TRUE, recursive = TRUE)
PKCFiles <- dir(file.path("pkcs"), pattern = ".pkc$",
                full.names = TRUE, recursive = TRUE)
...
SamplesMetadata = read_excel('annotation/Annotation_File.xlsx')
