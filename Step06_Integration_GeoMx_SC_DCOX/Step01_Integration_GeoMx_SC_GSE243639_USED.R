...
write.table(MetaData, paste0(outputDir, PI , '_MetaData.txt'), quote = F, row.names = F, sep = '\t', col.names = F)

#------------------------- CellType Annotations
CellTypeEnrichment = fread('/Users/isarnassiri/Documents/Parkinson/Data/GSE243639_RAW/Neurons.txt')
...
TP_profile = TP_profile[,which(colnames(TP_profile) %in% CellTypeEnrichment_subset$CELL_ID)]
...
fwrite(CellTypeEnrichment_subset, paste0(PI, '_CellTypeLabel_PD.txt'), quote = F, row.names = F, sep = '\t')
...
Barcode_Donor_All_subset = CellTypeEnrichment_subset[match(row.names(colData(sce_prediction_assigned)), CellTypeEnrichment_subset$CELL_ID),]
dim(Barcode_Donor_All_subset)
identical(Barcode_Donor_All_subset$CELL_ID, row.names(colData(sce_prediction_assigned)))
...
MetaData_Integrated = data.frame(Barcode = Barcode_Donor_All_subset$CELL_ID, CellType = colData(sce_prediction_assigned)$cell_type1, Donor = Barcode_Donor_All_subset$Samples)
...
## ---- [test] Perform aggregation of counts and metadata by subject and cell type.
...
subj_dds_transf[which(subj_dds_transf$feature == 'COBL'),]
...
subj_dds_transf$feature[which(subj_dds_transf$pvalue < 0.01)]
