...
Data <-
  readNanoStringGeoMxSet(dccFiles = DCCFiles,
                         ...
                         phenoDataSheet = "Annie_PD_Run_01_11Oct23_2023111",
                         phenoDataDccColName = "Sample_ID",
                         protocolDataColNames = c("aoi","roi","diseased", "NucleiCount"),
...
row.names(pData(target_Data))[which(pData(target_Data)$DetectionThreshold == '1-5%')]

# A08 and A02 were filtered out

test = sData(Data)
toMatch <- c("A02", "A08")
matches <- unique(grep(paste(toMatch,collapse="|"), test$SampleID, value=TRUE))

sum(test$NucleiCount[-which(test$SampleID %in% matches)])
sum(test$NucleiCount[-which(test$SampleID %in% matches | test$diseased == "diseased")]) #healty
sum(test$NucleiCount[-which(test$SampleID %in% matches | test$diseased != 'diseased')]) #with LBP
# > sum(test$NucleiCount[-which(test$SampleID %in% matches)])
# [1] 207
# > sum(test$NucleiCount[-which(test$SampleID %in% matches | test$diseased == "diseased")]) #healty
# [1] 146
...
#------------ I use standR package for filtering, therefore use the original number of genes in the metadata
...
boxplot(exprs(target_Data)[,1:12],
        col = "#9EDAE5", main = "Raw Counts",
        log = "y", names = gsub('.dcc|DSP-1001660016912-A-','',colnames(exprs(target_Data)[,1:12])), xlab = "Segment",
        ylab = "Counts, Raw")

boxplot(assayDataElement(target_Data[,1:12], elt = "q_norm"),
        col = "#2CA02C", main = "Q3 Norm Counts",
        log = "y", names = gsub('.dcc|DSP-1001660016912-A-','',colnames(exprs(target_Data)[,1:12])), xlab = "Segment",
        ylab = "Counts, Q3 Normalized")

boxplot(assayDataElement(target_Data[,1:12], elt = "neg_norm"),
...
spe2_subset <- addPerROIQC(spe2, rm_genes = TRUE)
...
plotRLExpr(spe2_subset, ordannots = "slide.name", assay = 2, color = segment)
plotRLExpr(spe2_subset, ordannots = "slide.name", assay = 2, color = area)
...
plotPairPCA(spe_ruv, precomputed = pca_results_ruv, color = segment, title = "RUV4, k = 5", n_dimension = 4)
...
# LB509TH vs TH (TH is reference)
contr.matrix <- makeContrasts(BvT = LB509TH - TH, levels = colnames(design))
